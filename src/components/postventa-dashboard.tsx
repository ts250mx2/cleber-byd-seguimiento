          ...
          {view === "dashboard" && <>
            ...
            <section className="dashboard-grid">
              ...
              <article className="panel pulse-panel">
                ...
                <div className="chart-wrap"><ResponsiveContainer width="100%" height={215}><BarChart data={chartData} margin={{ top: 16, right: 0, left: -28, bottom: 0 }}><CartesianGrid vertical={false} stroke="#e8eef3" /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: "#697f92", fontSize: 10 }} /><YAxis domain={[0, 100]} axisLine={false} tickLine={false} tick={{ fill: "#93a3b1", fontSize: 10 }} /><Tooltip cursor={{ fill: "#f4f8fb" }} contentStyle={{ border: "1px solid #dce7ef", borderRadius: 10, boxShadow: "0 10px 26px rgba(12,46,76,.1)", fontSize: 12 }} formatter={(value) => [`${value}%`, "Cumplimiento"]} /><Bar dataKey="value" radius={[6, 6, 0, 0]} barSize={24}>{chartData.map((entry) => <Cell key={entry.name} fill={entry.value < 50 ? "#536be8" : "#174b78"} />)}</Bar></BarChart></ResponsiveContainer></div>
